...
public final class V13Chapter4Data extends SavedData {
   ...
   public static V13Chapter4Data load(CompoundTag t,Provider p){V13Chapter4Data d=new V13Chapter4Data();int sourceSchema=t.contains("SchemaVersion")?Math.max(1,t.getInt("SchemaVersion")):1;d.stage=t.getString("Stage");if(d.stage.isBlank())d.stage=V13Chapter4Rules.LOCKED;
      ...
      readSet(t,"Participants",d.participants);readSet(t,"Acknowledgements",d.acknowledgements);d.migrate(t,sourceSchema);return d;}
   @Override public synchronized CompoundTag save(CompoundTag t,Provider p){t.putInt("SchemaVersion",SCHEMA_VERSION);t.putString("Stage",stage);t.putLong("StageTick",stageTick);t.putLong("LeftTick",leftTick);t.putLong("RightTick",rightTick);t.putString("LeftPlayer",leftPlayer);t.putString("RightPlayer",rightPlayer);
   ...
   private void migrate(CompoundTag t,int sourceSchema){
      StorySaveMigrationRules.Chapter4State clean=StorySaveMigrationRules.chapter4(testimonyMask,optionalMask);testimonyMask=clean.testimonyMask();optionalMask=clean.optionalMask();
      if(!t.contains("SiteBuilt")&&!BlockPos.ZERO.equals(archive))siteBuilt=true;
      if(participants.isEmpty())participants.addAll(StorySaveMigrationRules.participants(participants,Set.of(),leftPlayer,rightPlayer));
      acknowledgements.retainAll(participants);
